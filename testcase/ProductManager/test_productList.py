...
class TestLogin:
    ...
    @pytest.mark.parametrize('base_info,testcase', get_testcase_yaml('./testcase/ProductManager/checkInventory.yaml'))
    def test_check_inventory(self, base_info, testcase):
        allure.dynamic.title(testcase['case_name'])
        RequestBase().specification_yaml(base_info, testcase)

    @allure.story(next(c_id) + '删除购物车商品')
    @pytest.mark.parametrize('base_info,testcase', get_testcase_yaml('./testcase/ProductManager/delCart.yaml'))
    def test_del_cart(self, base_info, testcase):
        allure.dynamic.title(testcase['case_name'])
        RequestBase().specification_yaml(base_info, testcase)

    # 如果 addCart.yaml 作为单接口存在，也可以添加
    @allure.story(next(c_id) + '添加购物车')
    @pytest.mark.parametrize('base_info,testcase', get_testcase_yaml('./testcase/ProductManager/addCart.yaml'))
