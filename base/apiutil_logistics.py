...
class LogisticsRequestBase(object):
    """
    物流项目专用接口请求基类
    特点：
    1. 支持从响应头提取 Cookie（登录接口）
    2. 支持请求参数为列表（如调度派车接口）
    3. 增强了 replace_load 对列表的处理
    """
    ...
    def replace_load(self, data):
        """
        增强版数据替换解析，支持列表和基本类型
        原方法在处理列表时会返回字符串，现改为返回正确类型
        """
        # 记录原始类型，用于后续还原
        original_type = type(data)
        # 如果不是字符串，先转为JSON字符串以便替换
        if not isinstance(data, str):
            str_data = json.dumps(data, ensure_ascii=False)
        else:
            str_data = data

        # 循环替换所有 ${func(params)} 形式
        ...
        if original_type == dict:
            ...
            try:
                data = json.loads(str_data)  # 将字符串转回列表
            except json.JSONDecodeError:
                data = str_data
        else:
            # 其他类型（字符串、数字等）直接使用替换后的字符串，并尝试转回原类型
            if original_type == int and str_data.isdigit():
                data = int(str_data)
    ...
    def extract_cookie_from_response(self, response, cookie_key='cookie'):
        """
        从响应头中提取 Set-Cookie 并写入 extract.yaml
        :param response: requests.Response 对象
        :param cookie_key: 存入 extract.yaml 的键名
        """
        if 'Set-Cookie' in response.headers:
            cookie_value = response.headers.get('Set-Cookie')
            # 通常 Set-Cookie 包含多个属性，只取第一个分号前的实际值
            cookie_value = cookie_value.split(';')[0].strip()
            self.read.write_yaml_data({cookie_key: cookie_value})
            logs.info(f"从响应头提取到 Cookie：{cookie_key}={cookie_value}")
        else:
            logs.warning("响应头中未找到 Set-Cookie")

    def specification_yaml(self, case_info):
        """
        处理物流项目 yaml 测试用例，支持 Cookie 提取和列表参数
        :param case_info: list类型，实际取 case_info[0] 作为单条用例
        """
        ...
        try:
            ...
            for tc in case_info["testCase"]:
                ...
                if isinstance(val, str):
                    ...
                    validation = val
                # 弹出 validation，避免传给 run_main
                tc.pop('validation', None)
                ...
                # 替换请求参数中的变量（支持 params/data/json）
                ...
                # 特殊处理：如果当前是登录接口，从响应头提取 Cookie
                if 'login' in url.lower() or api_name.lower() == '登录':
                    self.extract_cookie_from_response(res)
